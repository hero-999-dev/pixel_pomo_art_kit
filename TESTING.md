...
**88 tests, all passing.** Verified 2026-07-25 on `main`, Python 3.14.4,
Pillow 12.2.0, `PIXEL_POMO_TOOLS` unset (i.e. against the sibling game checkout
the kit resolves relatively, `..\App\flutter\tools\gen_objects.py`).
...
- **`test_app_smoke.py` skips itself when there is no display.**
  ...
  headless machine (CI, a remote shell with no Windows session) all 16 of its
  tests report as **skipped**, not failed, and `discover` still exits 0. In
  this environment a real display was available (1024x768), so all 16 ran for
