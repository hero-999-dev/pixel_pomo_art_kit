...
**63 tests, all passing.** Verified 2026-07-25 against commit `a08865c` on
`art-kit-v1`, Python 3.14.4, Pillow 12.2.0, `PIXEL_POMO_TOOLS` unset (i.e.
...
- **`test_app_smoke.py` skips itself when there is no display.**
  ...
  headless machine (CI, a remote shell with no Windows session) all 6 of its
  tests report as **skipped**, not failed, and `discover` still exits 0. In
  this environment a real display was available (1024x768), so all 6 ran for
